...
**定位（与论文一致）**：经典 NP 定义存在悖论（存在量词对分支同一性无语义）；**在改变经典 NP 定义（最小分支次数＝本质维度 κ）的前提下，问题本质上成为 P_F 与 NP_F 的分离——本工程严格证明的是 `P_F ⊊ NP_F`（Lean 形式化，单一公理 V6）**。原版定义下的经典 P vs NP 问题因定义可能存在悖论，讨论并无意义。
