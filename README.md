...
- 这与论文立场一致：经典 NP 定义存在悖论（存在量词对分支同一性无语义，语义同一分支与语法两个分支不可区分）；**在改变经典 NP 定义（最小分支次数＝本质维度 κ）的前提下，问题本质上成为 `P_F` 与 `NP_F` 的分离——已严格证明的是 `P_F ≠ NP_F`（Lean 形式化，单一公理 V6）**。原版定义下的经典 P vs NP 问题因定义可能存在悖论，讨论并无意义。已知边界：NTM2 与 CBTM 的语义在特殊情形下不同（输入虚部模式与虚拟带 vbAt 不一致时 toCBTM 使用占位转移），该差异尚未处理；同构与接受保持只在规范复合串上成立。
