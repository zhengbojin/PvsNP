...
- 同构桥公理 `NTM2_solve_implies_IsNP_F`：CBTM ≅ NTM2 的同构桥
  ...
  `StructIsoNTM2CBTM` + `exists_CBTM_iso_NTM2` + 接受保持定理替代。
- `subsetSum_in_NP_F` 由两公理组装（theorem），供 FinalProof 使用。

约定（V1.2/V1.4）：在公理版下不构造具体 CBTM；最终版本以
...
/-- 实例 → Bool 串编码（target :: elements 的二进制 digits 展平）。 -/
def encodeInstBits (inst : SubsetSumInstance) : List Bool :=
  (inst.target :: inst.elements).flatMap (fun v => Nat.digits 2 v)

/-- NTM2 路径的分叉计数：路径中读符号为分叉符号（vb = true）的步数。 -/
def ntm2ForkCount (A : NTM2) (π : NTM2ComputationPath) : ℕ :=
  (π.filter (fun step => A.vb step.readSym)).length

/-- [公理 V1.4] 存在 NTM2 在 n（= 元素个数）次分叉后求解子集和：
    对每个非空实例，子集和成立当且仅当该 NTM2 有一条磁带语义的接受路径，
    且该路径的分叉次数恰为元素个数。 -/
axiom exists_NTM2_solves_subsetSum :
  ∃ A : NTM2, ∀ inst : SubsetSumInstance, inst.elements ≠ [] →
    (subsetSumHolds inst ↔
      ∃ π, ∃ cfg : NTM2Config A (encodeInstBits inst),
        TapeReachablePathNTM2 A (encodeInstBits inst) π cfg ∧
        cfg.state ∈ A.acceptStates ∧
        ntm2ForkCount A π = inst.elements.length)

/-- [公理 V1.2] CBTM ≅ NTM2 的同构桥：NTM2 求解 ⇒ IsNP_F。
    消去路径：`exists_CBTM_iso_NTM2`（CBTM.lean:787，定理：每个 NTM2
